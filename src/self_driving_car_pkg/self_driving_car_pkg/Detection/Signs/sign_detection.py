...
model_loaded = False
model = 0
sign_classes = ["speed_sign_30", "speed_sign_60", "speed_sign_90", "stop", "left_turn",
                "No_Sign"]  # Trained CNN Classes

sign_tracker = Tracker()


def image_for_keras(image):
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)  # Image everywhere is in rgb but Opencv does it in BGR convert Back
...
def sign_det_n_track(gray, frame, frame_draw):
    if sign_tracker.mode == config.DETECTION:
        ...
        if circles is not None:
            ...
            for i in circles[0, :]:
                ...
                try:
                    ...
                    if sign != 'No_Sign':
                        # 4i. Display Class
                        cv2.putText(frame_draw, sign, (end_point[0] - 80, start_point[1] - 10), cv2.FONT_HERSHEY_PLAIN, 1,
                                    (0, 0, 255), 1)
                        ...
                        # 4h. Else if Sign detected First time ... Update signs location and its detected count
                            ...
                            sign_tracker.known_centers_classes_confidence.append(
                                list(to_categorical(sign_classes.index(sign), num_classes=6)))
                    ...
                    if save_dataset:
                        ...
                        if (iter % 5) == 0:
                            ...
                            img_dir = os.path.abspath(
                                "self_driving_car_pkg/self_driving_car_pkg/data/live_dataset") + class_id

...
def BwareaOpen(img, MinArea):
    ...
    return cv2.drawContours(
        thresh, cnts_TooSmall, -1, 0, -1
    )  # [ contour = less then minarea contour, contourIDx, Colour , Thickness ]
...
def RetLargestContour(gray):
    ...
    if Max_Cntr_idx != -1:
        thresh = cv2.drawContours(
            thresh, cnts, Max_Cntr_idx, (255, 255, 255), -1
        )  # [ contour = less then minarea contour, contourIDx, Colour , Thickness ]
...
def RetLargestContour_OuterLane(gray, minArea):
    ...
    bin_img_dilated = cv2.morphologyEx(
        bin_img, cv2.MORPH_DILATE, kernel
    )  # Find the two Contours for which you want to find the min distance between them.
    bin_img_ret = cv2.morphologyEx(
        bin_img_dilated, cv2.MORPH_ERODE, kernel
    )  # Find the two Contours for which you want to find the min distance between them.
    bin_img = bin_img_ret
    #################################### TESTING SHADOW BREAKER CODE BY DILATING####################
...
def Ret_LowestEdgePoints(gray):
    ...
    cnts2 = cv2.findContours(
        Contour_TopBot_PortionCut, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )[1]
    ...
    for index, cnt in enumerate(cnts2):
        Lane_OneSide = np.zeros(gray.shape, dtype=gray.dtype)
        Lane_OneSide = cv2.drawContours(
            Lane_OneSide, cnts2, index, (255, 255, 255), 1
        )  # [ contour = less then minarea contour, contourIDx, Colour , Thickness ]
        Lane_TwoSide = cv2.drawContours(
            Lane_TwoSide, cnts2, index, (255, 255, 255), 1
        )  # [ contour = less then minarea contour, contourIDx, Colour , Thickness ]

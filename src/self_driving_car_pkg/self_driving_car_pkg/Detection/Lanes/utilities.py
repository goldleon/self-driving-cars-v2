...
def find_line_parameter(x1, y1, x2, y2):
    if (x2 - x1) != 0:
        slope = (y2 - y1) / (x2 - x1)
...
def average_2b_(Edge_ROI):
    ...
    if Edge_Binary_nz_pix[0].shape[0]:
        ...
        draw_points = (np.asarray([x_new, y_new]).T).astype(
            np.int32
        )  # needs to be int32 and transposed

        cv2.polylines(
            TrajectoryOnEdge, [draw_points], False, (255, 255, 255), 2
        )  # args: image, points, closed, color
        cv2.polylines(
            Lane_detected, [draw_points], False, (255, 255, 255), 2
        )  # args: image, points, closed, color

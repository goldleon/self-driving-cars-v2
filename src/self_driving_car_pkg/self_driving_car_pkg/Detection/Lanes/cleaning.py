...
def IsPathCrossingMid(Midlane, Mid_cnts, Outer_cnts):
    ...
    CarTraj_bottom_Pt = (
        int((Mid_bottom_Pt[0] + Outer_bottom_Pt[0]) / 2),
        int((Mid_bottom_Pt[1] + Outer_bottom_Pt[1]) / 2),
    )

    cv2.line(
        Ref_To_CarPath_Image,
        CarTraj_bottom_Pt,
        (int(Ref_To_CarPath_Image.shape[1] / 2), Ref_To_CarPath_Image.shape[0]),
        (255, 255, 0),
        2,
    )  # line from carstart to car path
    cv2.line(
        Midlane_copy,
        tuple(Mid_bottom_Pt),
        (Mid_bottom_Pt[0], Midlane_copy.shape[0] - 1),
        (255, 255, 0),
        2,
    )  # connecting midlane to bottom

    is_Ref_to_path_Left = (
        int(Ref_To_CarPath_Image.shape[1] / 2) - CarTraj_bottom_Pt[0]
    ) > 0
...
def GetYellowInnerEdge(OuterLanes, MidLane, OuterLane_Points):
    ...
    Outer_cnts = cv2.findContours(
        OuterLanes, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )[1]
    ...
    Ref = (
        0,
        0,
    )  # If MidContours are present use the first ContourPoint as Ref To Find Nearest YellowLaneContour
    if Mid_cnts:
    ...
    if Mid_cnts:
        ...
        if len(OuterLane_Points) == 2:
            ...
            if distance_(Point_a, Ref) <= distance_(Point_b, Ref):
                ...
                Closest_Index = 1
            Outer_Lanes_ret = cv2.drawContours(
                Outer_Lanes_ret, Outer_cnts, Closest_Index, 255, 1
            )
            Outer_cnts_ret = [Outer_cnts[Closest_Index]]

            # (b) If Correct outlane was detected =====================================
            IsPathCrossing, IsCrossingLeft = IsPathCrossingMid(
                MidLane, Mid_cnts, Outer_cnts_ret
            )
            if IsPathCrossing:
        ...
        if not np.any(OuterLanes > 0):
            ...
            Mid_lowP[1] = MidLane.shape[
                0
            ]  # setting mid_trajectory_lowestPoint_Row to MaxRows of Image
...
def ExtendShortLane(MidLane, Mid_cnts, Outer_cnts, OuterLane):
    ...
    if Mid_cnts and Outer_cnts:
        ...
        if BottomPoint_Mid[1] < Image_bottom:
            MidLane = cv2.line(
                MidLane,
                tuple(BottomPoint_Mid),
                (BottomPoint_Mid[0], Image_bottom),
                255,
                2,
            )
        ...
        if BottomPoint_Outer[1] < Image_bottom:
            if total_no_of_cnts_outerlane > 20:
                ...
                shift = 2
            RefLast10Points = Outer_cnts_Rowsorted[
                total_no_of_cnts_outerlane - shift : total_no_of_cnts_outerlane - 1 : 2,
                :,
            ]
            ...
            if len(RefLast10Points) > 1:  # Atleast 2 points needed to estimate a line
                ...
                if Ref_slope < 0:
                    ...
                    Ref_LineTouchPoint_col = (
                        OuterLane.shape[1] - 1
                    )  # Cols have lenth of ColLength But traversal is from 0 to ColLength-1
                    Ref_LineTouchPoint_row = (
                        Ref_slope * Ref_LineTouchPoint_col + Ref_yiCntercept
                    )
                Ref_TouchPoint = (
                    Ref_LineTouchPoint_col,
                    int(Ref_LineTouchPoint_row),
                )  # (col ,row)
                Ref_BottomPoint_tup = tuple(BottomPoint_Outer)
                OuterLane = cv2.line(
                    OuterLane, Ref_TouchPoint, Ref_BottomPoint_tup, 255, 2
                )
                ...
                if Ref_LineTouchPoint_row < Image_bottom:
                    ...
                    OuterLane = cv2.line(
                        OuterLane, Ref_TouchPoint, Ref_TouchPoint_Ref, 255, 3
                    )

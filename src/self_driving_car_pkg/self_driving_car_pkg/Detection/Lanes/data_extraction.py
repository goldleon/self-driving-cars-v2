...
from .utilities import cord_sort, find_lane_curvature
...
def LanePoints(midlane, outerlane, offset):
    ...
    outer_cnts = cv2.findContours(
        outerlane, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
    )[1]
    ...
    if mid_cnts and outer_cnts:
        ...
        traj_btm_pt = (
            int((m_rows_btm_pt[0] + o_rows_btm_pt[0]) / 2) + offset,
            int((m_rows_btm_pt[1] + o_rows_btm_pt[1]) / 2),
        )
        traj_top_pt = (
            int((m_rows_top_pt[0] + o_rows_top_pt[0]) / 2) + offset,
            int((m_rows_top_pt[1] + o_rows_top_pt[1]) / 2),
        )
...
def EstimateNonMidMask(MidEdgeROi):
    Mid_Hull_Mask = np.zeros(
        (MidEdgeROi.shape[0], MidEdgeROi.shape[1], 1), dtype=np.uint8
    )
...
def FetchInfoAndDisplay(Mid_lane_edge, Mid_lane, Outer_Lane, frame, Offset_correction):
    ...
    curvature = find_lane_curvature(
        Traj_lowP[0], Traj_lowP[1], Traj_upP[0], Traj_upP[1]
    )
    ...
    Lane_drawn_frame[ProjectedLane == 255] = Lane_drawn_frame[ProjectedLane == 255] + (
        0,
        100,
        0,
    )
    Lane_drawn_frame[Outer_Lane == 255] = Lane_drawn_frame[Outer_Lane == 255] + (
        0,
        0,
        100,
    )  # Outer Lane Coloured Red
    Lane_drawn_frame[Mid_lane == 255] = Lane_drawn_frame[Mid_lane == 255] + (
        100,
        0,
        0,
    )  # Mid Lane Coloured Blue
    Out_image = Lane_drawn_frame

    # 8. Draw Cars direction and Lanes direction and distance between car and lane path
    cv2.line(
        Out_image,
        (int(Out_image.shape[1] / 2), Out_image.shape[0]),
        (int(Out_image.shape[1] / 2), Out_image.shape[0] - int(Out_image.shape[0] / 5)),
        (0, 0, 255),
        2,
    )
    ...
    if Traj_lowP != (0, 0):
        cv2.line(
            Out_image,
            Traj_lowP,
            (int(Out_image.shape[1] / 2), Traj_lowP[1]),
            (255, 255, 0),
            2,
        )  # distance of car center with lane path
